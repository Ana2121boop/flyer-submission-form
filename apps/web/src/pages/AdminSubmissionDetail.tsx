          ...
          sortedPages.map((pageNum) => (
            <div key={pageNum} className="mb-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {productsByPage.get(pageNum)!.map((p) => <ProductTile key={p.id} product={p} />)}
              </div>
            </div>
          ))
        )}
...
function ProductTile({ product }: { product: AdminProduct }) {
  const categoryName = product.category?.name ?? product.categoryOther ?? null;
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-3">
      <div className="flex gap-3">
        {product.imageUrl ? (
          <img src={product.imageUrl} alt="" className="w-20 h-20 object-cover rounded border border-slate-200 shrink-0" />
        ) : (
